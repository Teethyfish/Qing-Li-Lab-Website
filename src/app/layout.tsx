...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const [personalNotices, announcementNotices, unreadAnnouncementCount] = userId
    ? await Promise.all([
        prisma.notification.findMany({
          ...
          take: 3,
          select: { id: true, title: true, message: true, readAt: true, createdAt: true, document: { select: { createdBy: { select: { name: true, email: true } } } } },
        }),
  ...
  const recentNotices = [
    ...personalNotices.map((notice) => ({
      ...
      uploaderName: notice.document ? notice.document.createdBy?.name?.trim() || notice.document.createdBy?.email || "" : null,
